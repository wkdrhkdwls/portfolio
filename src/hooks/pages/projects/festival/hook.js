  ...
  const titleData = {
    ...
    sections: [
      ...
      },
      {
        subtitle: "2. 개발 과정",
        description:
          "약 2주간의 짧은기간의 프로젝트였고 백엔드 개발자 없이 프론트엔드 개발자로만 이루어진 팀이었기에 어떻게 많은 데이터를 다루고 레이아웃을 구현할까에 대한 많은 고민을 하였습니다. 시간이 많지 않았기에 주에 3~4번의 회의를 거치고 현업 디자이너가 프로젝트에 도움을 주어 문제없이 진행할 수 있었으며, 이미지는 많지 않기에 assets 폴더에 텍스트 데이터는 hooks폴더에 각각 컴포넌트마다 hook.js를 생성하여 그 안에 데이터를 넣어 하드코딩을 방지하는 방식으로 진행했습니다.",
      },
      {
        subtitle: "3. 기술 스택",
        description:
      ...
      },
      {
        subtitle: "5. 인사이트",
        description:
          "처음에는 CSS프레임워크는 독립된 브랜치에서 자유롭게 사용해도 상관없겠다고 생각했지만, 후반부에 수정할 부분이 많아지며 CSS 프레임워크도 통일해서 사용하는 것이 좋겠다고 느꼈고, 처음으로 TL의 역할로 브랜치들을 Merge하고 팀원들을 끌어가는 과정에서 어려움을 많이 겪었습니다. 추가적으로 문제가 많은 React Fullpage를 Js Fullpage로 변경하고, CSS 프레임워크를 하나로 통합하는 보수 작업을 할 예정입니다.",
      },
    ],
  }
